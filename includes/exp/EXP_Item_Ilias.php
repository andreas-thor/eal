...
class EXP_Item_Ilias extends EXP_Item {
	...
	private function getQuestionType (EAL_Item $item): string {
		if ($item instanceof EAL_ItemSC) return 'SINGLE CHOICE QUESTION';
		if ($item instanceof EAL_ItemMC) return 'MULTIPLE CHOICE QUESTION';
		if ($item instanceof EAL_ItemFT) return 'TEXT QUESTION';
	}
	
	private function getQuestionIdent (EAL_Item $item): string {
		if ($item instanceof EAL_ItemSC) return 'MCSR';
		if ($item instanceof EAL_ItemMC) return 'MCMR';
		if ($item instanceof EAL_ItemFT) return 'TEXT';
	}
	
	
	private function getResponseElement (DOMDocument $dom, EAL_Item $item): DOMElement {
		if ($item instanceof EAL_ItemSC) return $this->createResponseElementForChoice($dom, $item, 'Single');
		if ($item instanceof EAL_ItemMC) return $this->createResponseElementForChoice($dom, $item, 'Multiple');
		if ($item instanceof EAL_ItemFT) return $this->createResponseElementForText($dom, $item);
	}
	
	private function getResprocessingElement (DOMDocument $dom, EAL_Item $item): DOMElement {
		if ($item instanceof EAL_ItemSC) return $this->createResprocessingElementForChoice($dom, $item);
		if ($item instanceof EAL_ItemMC) return $this->createResprocessingElementForChoice($dom, $item);
		if ($item instanceof EAL_ItemFT) return $this->createResprocessingElementForText($dom, $item);
	}
	...
	private function createQTI ($itemids) {
		...
		foreach ($itemids as $item_id) {
			...
			assert(($item instanceof EAL_ItemSC) || ($item instanceof EAL_ItemMC) || ($item instanceof EAL_ItemFT));
			...
			$xml_FL->appendChild ($this->getResponseElement($dom, $item));
			$xml_PR->appendChild ($xml_FL);
			$xml_IT->appendChild ($xml_PR);

			
			/* Resprocessing */
			$xml_IT->appendChild ($this->getResprocessingElement($dom, $item));
	...
	private function createResprocessingElementForChoice (DOMDocument $dom, EAL_Item $item): DOMElement {
		...
		for ($index=0; $index<$item->getNumberOfAnswers(); $index++) {
			foreach (array (1, 0) as $checked) {
				...
				$xml_VE->setAttribute ("respident", $this->getQuestionIdent($item));
	...
	private function createResprocessingElementForText (DOMDocument $dom, EAL_ItemFT $item): DOMElement {
		
		$xml_RP = $dom->createElement('resprocessing');
		$xml_RP->setAttribute('scoremodel', 'HumanRater');
		
		$xml_OC = $dom->createElement('outcomes');
		$xml_DV = $dom->createElement('decvar');
		$xml_DV->setAttribute('maxvalue', $item->getPoints());
		$xml_DV->setAttribute('minvalue', '0');
		$xml_DV->setAttribute('vartype', 'Integer');
		$xml_DV->setAttribute('varname', 'WritingScore');
		$xml_OC->appendChild ($xml_DV);
		$xml_RP->appendChild ($xml_OC);
		...
		$xml_RC = $dom->createElement('respcondition');
		$xml_CV = $dom->createElement('conditionvar');
		$xml_OT = $dom->createElement('other', 'tutor_rated');
		$xml_CV->appendChild($xml_OT);
		$xml_RC->appendChild($xml_CV);
		$xml_RP->appendChild ($xml_RC);
	...
	private function createResponseElementForChoice (DOMDocument $dom, EAL_Item $item, string $rcardinality): DOMElement {
		...
		$xml_RL->setAttribute('ident', $this->getQuestionIdent($item));
		$xml_RL->setAttribute('rcardinality', $rcardinality);
	...
	private function createResponseElementForText (DOMDocument $dom, EAL_ItemFT $item): DOMElement {
		
		$xml_RF = $dom->createElement("render_fib");
		$xml_RF->setAttribute ('prompt', 'Box');
		$xml_RF->setAttribute ('fibtype', 'String');
		
		$xml_RL = $dom->createElement("response_label");
 		$xml_RL->setAttribute ('ident', 'A');
 		$xml_RF->appendChild ($xml_RL); 		
		
 		$xml_RS = $dom->createElement("response_str");
		$xml_RS->setAttribute('ident', 'TEXT');
		$xml_RS->setAttribute('rcardinality', 'Ordered');
		
		$xml_RS->appendChild ($xml_RF);
		return $xml_RS;
	}
